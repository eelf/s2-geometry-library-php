...
class S2Polygon implements S2Region {
     ...
     */
        ...
        if (!$this->hasHoles && !$b->hasHoles) {
            for ($j = 0; $j < $b->numLoops(); ++$j) {
                if (!$this->anyLoopContains($b->loop($j))) {
                    return false;
                }
            }
            return true;
...
// only if some pair of loop regions intersect.
        ...
        if (!$this->hasHoles && !$b->hasHoles) {
            for ($i = 0; $i < $this->numLoops(); ++$i) {
                for ($j = 0; $j < $b->numLoops(); ++$j) {
                    if ($this->loop($i)->intersects($b->loop($j))) {
                        return true;
                    }
                }
            }
...
// an intersection.
     ...
     * int crossing,
     * List
     * <ParametrizedS2Point> intersections) {
     * if (crossing > 0) {
     ...
     *
     * List
     * <ParametrizedS2Point> intersections = Lists.newArrayList();
